...
- **Docker image**: every `v*` release tag also runs `.github/workflows/docker-build-push.yml`, publishing the multi-arch `hycanvas/hycanvas` image as `:latest`, `:development`, and `:<tag>`. Between releases, pushing to the `docker/build/latest` or `docker/build/development` branch (for example `git push origin development:docker/build/development`) publishes an image-only update to the matching channel.
