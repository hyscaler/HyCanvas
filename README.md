...
- **Docker image**: every `v*` release tag also runs `.github/workflows/docker-build-push.yml`, publishing the multi-arch `hycanvas/hycanvas` image as `:latest`, `:development`, and `:<tag>`, and advancing the `docker/build/latest` and `docker/build/development` branches to the released commit so they always reflect the published channels. Between releases, pushing to one of those branches (for example `git push origin development:docker/build/development`) publishes an image-only update to the matching channel.
