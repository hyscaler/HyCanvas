...
| PDF export | Built | `ExportDialog.tsx` jsPDF; `backend/internal/render/pdf.go` | Ships. The Go encoder now emits an accessibility structure tree and embeds images (FR-22); the dialog's own PDF is still a raster of each page, so it carries neither text nor tags. |
...
| Accessibility-tagged PDF export (PDF-UA) | Partial | `backend/internal/render/{pdftag,pdfimage}.go`; `render.ToDeckPDF`; `GET /designs/{id}/render.pdf?page=all` | The Go encoder emits a structure tree (Document -> per-slide Sect -> P/Figure), honors `readingOrder` independent of z-order, carries `altText` as `/Alt`, artifacts decorative content, and embeds images (JPEG passthrough, Flate + soft mask otherwise). Gap: the export dialog's PDF still rasterizes through jsPDF, and the Go encoder has no font embedding, so tagged text renders in the base-14 faces. P2 (FR-22). |
