...
The honest gaps: there is no PPTX (or Keynote/Google Slides/ODP) import or export at all, so existing decks cannot migrate in or out and PowerPoint-user interop is blocked; the slide master/layout/placeholder model and the swappable `Theme` record ship (FR-3/FR-4, schema v11), though no editor UI adopts them yet; the true second-display presenter view ships (FR-15: audience window + BroadcastChannel, wall clock, teleprompter); the transition compositor is now a pure `@hc/engine` helper (FR-13 shipped), and the web player and animated export (APNG/GIF) now consume it, so a deck exports its whole playthrough with transitions; the server MP4 path is still a static per-page render with no animation/transitions/audio (the Go side has no animation core), and there is no present-and-record; there is no present-and-record, no live audience join link / Q&A / polls / reactions / captions, no charts bound to live data, no bulk data-merge, and the alt-text/reading-order model ships (FR-29, schema v12) with the checker honoring it, though the Reading Order pane and tagged-PDF export remain. AI lacks whole-deck translation, AI speaker notes, and document/URL/file-to-deck ingestion. This spec is the forward-looking plan for all of that.
...
- FR-5 (sections + section-aware nav shipped; grid/outline overview pending): Slides group into named, colored, collapsible sections; a grid/outline overview editing view lets an author reorder the narrative; present navigation is section-aware.
