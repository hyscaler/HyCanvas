...
| Slide master / layout hierarchy (master -> layouts -> slides) | Built | `SlideMaster`/`SlideLayout`/`Placeholder` + `Page.layoutId` in `schema.ts` (v11); `editor.ts` `savePageAsLayout`/`applyLayoutToPage`/`updateLayoutFromPage`/`syncLayoutPages`; `PropertiesPanel` "Slide layout" section | Materialization model: save any page as a layout (auto master, title = largest text), link pages to it, edit-and-sync pushes background + placeholder changes to every linked page. Applying is idempotent (placeholder-tagged nodes) and never deletes page content (FR-3). Whole-deck extraction ships too (F28 T20): one action decomposes every page into slots (largest text = title, geometry-derived body/content with capacity hints, picture/chart/media slots, decoration ignored), dedupes near-identical pages on a coarse grid, optionally corrects roles with a vision pass over the rendered page (one self-review round, heuristic fallback on any failure), and verifies every capacity hint against a simulated max fill (qualityCheck-gated) before installing - so an imported PPTX becomes a reusable layout set that layout-grounded generation targets automatically. |
...
| Theme record (palette + heading/body font pair + effect styles, swappable) | Built | `Theme` record in `schema.ts` (v11); `PropertiesPanel.tsx` `DeckThemeSection` (seed themes, brand-kit bridge); `store/editor.ts` `setDeckTheme` (exact slot remap, one undo step); `aistudio/src/themeGen.ts` | The theme picker ships (F28 T19): curated seed palettes, create-from-brand-kit, and an adopted theme restyles content painted with the previous theme's exact slot colors and fonts in one undoable swap. |
...
| Outline-first editable flow | Built | `aistudio/src/outline.ts` + `outlineEdit.ts` (`sanitizeEditedOutline`, `dialsClause`); `EditorPanels.tsx` review card; `generate.go` `Outline` | A gated generateDesign shows the outline for review inside the confirmation: edit titles/points, reorder, add/remove pages, generation dials (density/tone/audience/scenario), regenerate, and a skip affordance; Generate proceeds with the edited outline with no second model call (T09). |
...
| AI theme generation as a swappable Theme | Built | `@hc/aistudio` `themeGen.ts` (`buildGeneratedTheme`, OKLCH ladder repair); assistant tool `generateTheme` | One structured call proposes palette + font pair, validated hex-strict and repaired to WCAG AA deterministically; the result is written as the swappable `DesignFile.theme` (F28 T19). Deck generation also stamps its `DeckTheme` as the file theme. |
...
| Brand kit (logos/palette/fonts, multiple kits) | Partial | `BrandPanel` + `applyBrandFixes` `editor.ts:3501`; `DesignFile.palette`/`fonts`; `POST /ai/brand-from-url` (`httpapi/brandfromurl.go`) | Brand grounding + apply-brand ship, and a kit now drafts itself from a company domain (F28 T21): the SSRF-gated scan surfaces logo candidates, a frequency-ranked palette (one optional model pass re-ranks brand vs chrome, choosing only from observed colors), and font guesses, all behind an explicit confirm step in `BrandPanel` - nothing saves until confirmed, and the chosen logo imports as a workspace asset. A centralized multi-kit Brand Hub with brand-locked templates is not formalized. |
...
| AI theme generation from prompt / brand inputs | Built | `@hc/aistudio` `themeGen.ts`; `generateTheme` assistant tool; brand-kit bridge in `DeckThemeSection` | Prompted themes and brand-derived themes both land as the swappable `Theme` record (F28 T19). |
