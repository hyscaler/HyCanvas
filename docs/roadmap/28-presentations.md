...
Audited against the code: `packages/schema/src/schema.ts` (`Page` present fields at 1517-1549, `PageTransition` 449-459, `EntrancePreset`/`ExitPreset`/`EmphasisPreset` 333-341, `AnimationClip`/`AnimationStartMode` 343-368, `Keyframe`/`KeyframeTrack` 370-405, `NodeAnimation` 407-421 on `NodeBase.animation` 622, `Interaction`/`InteractionAction` 423-447, `ImageMotion` 461-469, `ChartNode`/`TableNode`, `VideoNode`/`AudioNode` 1092-1131) + `migrate.ts` (`CURRENT_SCHEMA_VERSION = 9`); `packages/engine/src/{animation,pose}.ts` (`entrancePatch`/`exitPatch`/`emphasisPatch`/`customPatch`/`imageMotionPatch`/`evalEasing`/`cubicBezierEase`, `poseDesignAt`/`sequenceStarts`/`revealEntranceText`/`pageAnimationDuration`); `frontend/src/components/editor/PresentMode.tsx` (`renderTransition`/`morphPlan`/`lerpNode`, `runInteraction`, autopilot, laser/pen/spotlight/zoom/blank overlay, `JumpPalette`, `PresenterHud`, `RehearsalTimer`); `frontend/src/lib/present.ts` (`visibleIndices`/`seekVisible`/`dwellMs`/`spotlightGeom`/`RehearsalTimer`/`formatClock`); `frontend/src/components/editor/{PagesBar,PropertiesPanel,ExportDialog,EditorPanels,SharedViewer,ShareDialog}.tsx`; `frontend/src/store/editor.ts` (`addPage`/`duplicatePage`/`movePage`/`setPageNotes`/`setPageTransition`/`setNodeAnimation`/`magicAnimatePage`/`playAnimations`/`buildDeckFromOutline`/`magicResizePages`/`insertChartData`/`applyBrandFixes`); `packages/aistudio/src/{outline,deck,layout,theme,transform,assistant,quality,prompts}.ts`; `packages/export/src/{apng,gif,lottie,svg,dimensions}.ts`; backend `internal/render/{raster,pdf,svg,video}.go`, `internal/aistudio`, `internal/sharing`, `internal/comments`, `internal/engagement` (`useViewBeat.ts` + insights), `internal/realtime/*.go`, `internal/persistence/file.go`.
...
The honest gaps: there is no PPTX (or Keynote/Google Slides/ODP) import or export at all, so existing decks cannot migrate in or out and PowerPoint-user interop is blocked; there is no slide master/layout/placeholder model and no swappable theme record (each `Page` is standalone); the presenter HUD is a same-window overlay, not a true second display; the transition compositor lives in `PresentMode.tsx` React code (present-only, not pure, not exportable), so animated export is single-page APNG/GIF only and the server MP4 path is a static per-page render with no animation/transitions/audio; there is no present-and-record, no live audience join link / Q&A / polls / reactions / captions, no charts bound to live data, no bulk data-merge, and no alt-text/reading-order/accessibility checker on the slide surface. AI lacks whole-deck translation, AI speaker notes, and document/URL/file-to-deck ingestion. This spec is the forward-looking plan for all of that.
...
| Slide master / layout hierarchy (master -> layouts -> slides) | Not started | n/a | No master/layout/placeholder model; each `Page` is standalone. Backbone for templating at scale, accessible reading order, AI layout grounding, and brand restyle (three workstreams depend on it). P0 (FR-3). |
| Placeholders + built-in layouts (title/content/two-content/comparison/picture) | Not started | n/a | No placeholder concept, hand-placed boxes only. Required for guaranteed accessible slide titles + reading order and for layout-grounded AI. P0 (FR-3). |
| Theme record (palette + heading/body font pair + effect styles, swappable) | Partial | `DesignFile.palette`/`fonts`; AI `DeckTheme` in `aistudio/src/{outline,theme}.ts` | File-level palette/fonts and AI `DeckTheme`s exist but there is no first-class swappable `Theme` that restyles a whole deck. P0 (FR-4). |
| Slide sections / chapters (group, collapse, reorder) | Not started | n/a | `DesignFile.pages` is a flat `Page[]`. Add `Page.sectionId` or `sections[]` + `PagesBar` grouping + section-aware present nav (FR-5). |
| Grid / outline overview editing view (light table) | Not started | n/a | The `JumpPalette` is a present-mode grid; there is no editing-surface grid/outline overview to reorder the narrative (Figma Slides grid view, Keynote light table). FR-5. |
...
| Magic Charts (NL / table text -> editable chart) | Built | `aistudio/src/transform.ts` `normalizeChartSpec`/`chartSpecJsonSchema`; `EditorPanels.tsx:2337` `ChartPanel`; `editor.ts:2333` `insertChartData`; `generate.go:140` | NL or pasted table -> validated `ChartSpec` -> native `ChartNode`. |
| Live data-linked charts (Sheets/CSV/connector, refresh-on-present) | Not started | n/a | `ChartNode` data is static; no `DataBinding`. Google Slides<->Sheets and Canva Sheets live-link is the bar. P1 (FR-8). |
| Chart from uploaded data file (CSV/XLSX) | Not started | n/a | Magic Charts takes NL/pasted text only; no file upload to chart. P2 (FR-8). |
...
| Bulk create / data-merge (CSV row -> slide) | Not started | n/a | No mail-merge slides from a data source (Canva Bulk Create). Valuable for catalog/report decks. P2 (FR-9). |
...
| Feature | Status | Where (code) | Notes |
| --- | --- | --- | --- |
| Page transitions (9 types incl. flip/zoom/wipe) | Built | `PresentMode.tsx` `renderTransition` (~1465-1679); `PageTransition` `schema.ts:449-459`; `PageTransitionSection` | fade, dissolve, slide, push, morph-lite, wipe, flip, zoom, morph. Direction for slide/push/wipe; per-page duration; transition belongs to the arriving slide (on-enter only). |
| Transition compositor is pure + exportable (in `@hc/engine`) | Partial | `renderTransition` lives in `PresentMode.tsx` React code | Present-only; export cannot render slide-to-slide transitions and the web player cannot either. Must lift into a pure `@hc/engine` helper. P0 dependency for the web player and deck-to-video (FR-13). |
...
| Feature | Status | Where (code) | Notes |
| --- | --- | --- | --- |
| Audience-facing live present share (join link / broadcast) | Not started | n/a | No audience join link or live slide broadcast. The `/realtime` relay + presence (`packages/realtime`, `backend/internal/realtime`) exists for co-editing and can carry present-along frames. Includes remote viewers following the in-room presenter (Keynote Live). P1 (FR-19). |
...
| Live Q&A with upvote / moderation | Not started | n/a | No audience question submission/upvote. Slido/Google Slides bar. Build on the relay; surfaces in the HUD. P1 (FR-20). |
| Live polls / word clouds / quizzes | Not started | n/a | No audience-response slides. Mentimeter/Figma Slides bar. Needs a `PollNode` + live tally over the relay. P2 (FR-20). |
| Live emoji reactions per slide | Not started | n/a | Whiteboard already has ephemeral reactions over realtime; reuse for per-slide audience reactions (Canva Live bar). P2 (FR-20). |
...
| MP4 video export (server) | Partial | `backend/internal/render/video.go`; `httpapi/export.go` POST `/designs/{id}/export/video`; GET `render.mp4` | ffmpeg image2pipe -> libx264, job-based, but STATIC per-page render (default 3s). No timeline compositing, element animation, transitions, or narration. Not surfaced in `ExportDialog`. Needs per-frame `poseDesignAt` sampling + the ported transition compositor. P1 (FR-19). |
| Deck-to-video with synced animations + transitions across all slides | Not started | n/a | No continuous full-deck video. Animated export is single-page; transitions are present-only. Keynote/PowerPoint/Canva all ship this. Depends on the pure exportable compositor (FR-13). P1 (FR-19). |
| Present-and-record (MediaRecorder: slide + narration + camera + ink/laser) | Not started | n/a | No live-run capture. Browser `MediaRecorder` over the slide canvas `captureStream` + mic + camera bubble; ungated vs Canva/Pitch/Prezi Video. P1 (FR-19). |
...
| Outline-first editable flow | Built | `aistudio/src/outline.ts` `normalizeOutline`/`outlineJsonSchema`; `StudioPanel`; `generate.go:46` | Reorder/edit/add/remove before generating; skip-outline fast path. |
...
| Magic Switch (deck -> doc/social/poster) | Built | `aistudio/src/transform.ts` `deriveOutline`/`switchOutline`; `EditorPanels.tsx:2280` `TransformPanel` | Deterministic re-shape to a new type, re-laid-out native. |
...
| Critique-and-improve (design critique + harmonize/tidy) | Built | `aistudio/src/quality.ts` `qualityCheck`; `EditorPanels.tsx:2382` `ImprovePanel`; `generate.go:189` | Deterministic contrast/overflow/overlap flags + AI critique. No per-issue accept/reject list (FR-23). |
...
| Whole-deck translation | Not started | n/a | Only per-node translate (`EditorPanels.tsx:3020`). No walk-every-slide translate preserving layout. Gamma/Canva/Copilot do full-deck multilingual. Needs an all-text-node iterator. P1 (FR-23). |
| AI speaker-notes generation | Not started | n/a | `Page.notes` ships (`schema.ts:1526`), `setPageNotes` (`editor.ts:526`), never AI-populated. Emit notes alongside copy-polish or as an assistant tool. Copilot/Plus AI bar. P1 (FR-23). |
| Document / URL / file -> deck ingestion | Not started | n/a | Generation source is a prompt string only. No doc/URL/file pipeline (Gamma import, Copilot create-from-file, Plus AI from Google Doc). PDF/SVG import exists for assets, not as a generation source. P1 (FR-23). |
...
| Read-only shared viewer | Partial | `frontend/src/components/SharedViewer.tsx` | Stacks page canvases (scroll), client-only, latest snapshot. NOT a present mode: no slide-by-slide nav, fullscreen, notes, arrow advance, or live-follow. Must become a real web player. |
| Web player link with slide navigation + fullscreen + transitions | Not started | n/a (SharedViewer is a scroll stack) | No Gamma/Pitch-style responsive web player. Extend `SharedViewer`/`pages/shared.tsx` with engine-rendered slide nav + fullscreen + transitions (needs the pure exportable compositor, FR-13). P1 (FR-26). |
...
| Feature | Status | Where (code) | Notes |
| --- | --- | --- | --- |
| PPTX import (high-fidelity, editable: text/animations/fonts/masters) | Not started | n/a (only SVG/text-PDF/clipboard import) | THE interop battleground and the literal blocker to PowerPoint-user adoption. New importer producing `@hc/schema` nodes (new node types need a migration). P0 (FR-27). |
| PPTX export (native PowerPoint round-trip) | Not started | n/a | No pptx code anywhere. Editable round-trip (slides -> shapes/text/notes/transitions/masters). New `@hc/*` core + Go encoder + job route. P0 (FR-27). |
...
| PDF export | Built | `ExportDialog.tsx` jsPDF; `backend/internal/render/pdf.go` | Ships. Accessibility-tagged PDF (PDF-UA) is a further differentiator (FR-22). |
...
| Accessibility-tagged PDF export (PDF-UA) | Not started | n/a (PDF export is untagged) | PowerPoint exports tagged PDF preserving structure. Needs tag emission in the PDF encoder. P2 (FR-22). |
...
- FR-4: A first-class swappable `Theme` record (12-slot color palette + heading/body font pair + effect styles + variants) restyles the whole deck in one action; AI theme generation produces a `Theme` the deck can adopt.
- FR-5: Slides group into named, colored, collapsible sections; a grid/outline overview editing view lets an author reorder the narrative; present navigation is section-aware.
...
- FR-8: `ChartNode` gains an optional `DataBinding` (sheet/CSV/connector source + ref + `refreshOnPresent`); charts refresh from the source on demand and optionally on present, caching the last-fetched data in the file; a CSV/XLSX upload also produces a native chart.
- FR-9: A bulk data-merge pipeline maps rows of a data source to one slide per row over a template (text + image placeholders), producing native editable slides.

Motion and transitions:
- FR-10: A page-level animation build-order timeline shows every element's entrance order, start-mode, delay, and duration on one strip, draggable to reorder and overlap (PowerPoint Animation Pane / Keynote Build Order parity); an apply-to-all op sets a transition or animation across slides.
...
- FR-13: The slide-to-slide transition compositor is lifted out of `PresentMode.tsx` into a pure `@hc/engine` helper so editor preview, present mode, the web player, and headless export all render transitions identically; transitions gain selectable per-transition easing and, on the GPU path, 3D/cinematic types.
...
- FR-26: A real web player link (engine-rendered slide nav + fullscreen + transitions via FR-13) replaces the scroll-stack `SharedViewer`; multiple named per-audience tracked links with passcodes carry per-link, per-slide dwell analytics; a public iframe/web embed renders the deck player; all keep the existing share-link security guards.
...
- FR-29: `NodeBase` gains optional `altText` and `decorative`; `Page` gains a reading-order/tab-order list independent of z-order; an in-canvas WCAG 2.2 AA checker flags low-contrast, missing alt text, and unreadable text at authoring time with one-click fixes; slide titles are guaranteed via the title placeholder (FR-3); a presentations ACR/VPAT is published.

## 7. Data model / schema changes

All present node types and properties are added to the open file format per the schema-is-contract rule: extend the `NodeType` union and `KNOWN_NODE_TYPES` in `packages/schema/src/schema.ts`, define the interface + Zod schema with `...nodeBaseFields, type: z.literal("...")`, add it to the `KnownNode` union and discriminated `NodeSchemaByType`, give it a default in `factory.ts`, register a forward migration step in `migrate.ts` keyed on the source version, and bump `CURRENT_SCHEMA_VERSION` (currently 9). Additive node types and optional fields need only a version bump, because older files omit them and `UnknownNode.raw` preserves a newer client's nodes losslessly, so additive rollout is safe across mixed-version clients. Two coupling rules apply to every version bump: (1) raise the Go mirror `currentSchemaVersion` in `backend/internal/persistence/file.go` in the same change, or the write boundary `persistence/validate.go` rejects the newer file with a 422 (`ErrInvalidFile`) and nothing persists (the rejection is on the top-level `schemaVersion` field, not on per-node round-trip); purely-additive bumps need no new Go migration step. (2) Append a one-line entry to the schema-version-history doc-comment above `CURRENT_SCHEMA_VERSION` in `schema.ts`.
