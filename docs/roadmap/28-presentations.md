...
| PDF export | Built | `ExportDialog.tsx` jsPDF; `backend/internal/render/pdf.go` | Ships. The Go encoder now emits an accessibility structure tree (FR-22); the dialog's own PDF is still a raster of each page, so it carries neither text nor tags. |
...
| Accessibility-tagged PDF export (PDF-UA) | Partial | `backend/internal/render/pdftag.go`; `render.ToDeckPDF`; `GET /designs/{id}/render.pdf?page=all` | The Go encoder emits a structure tree (Document -> per-slide Sect -> P/Figure), honors `readingOrder` independent of z-order, carries `altText` as `/Alt`, and artifacts decorative content. Gap: the export dialog's PDF still rasterizes through jsPDF, and the Go encoder draws no image nodes, so the tagged path cannot yet replace it. P2 (FR-22). |
