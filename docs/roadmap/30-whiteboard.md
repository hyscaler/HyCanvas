...
The honest remaining gaps: Phase 2 facilitation is essentially COMPLETE (spotlight/summon/take-control, server-authoritative voting, timer presets + end alert, named views/agenda + deep-links, presence-scale interest culling, the emoji/vote stamp, PRIVATE MODE (`meta.privateMode` hidden-until-reveal rounds in `WhiteboardSurface`), the PROTECTED facilitator lock with mid-session HANDOFF (`hub.HandleProtect`/`HandleFacilitator`, FR-16), and kick/ban moderation (`hub.HandleModerate`, FR-32) all ship; first-run onboarding polish and anonymous-guest rate limiting remain). Phase 3 (AI on the canvas) now SHIPS its core: diagram-from-prompt (assistant tool `generateDiagram` -> native stickies + connectors via `layoutFlowchart`/`layoutMindMap`, one undo step), sticky clustering into labeled theme frames (`clusterStickies` -> `applyStickyClusters`) and board summarize (`summarizeStickies` -> a canvas text note), all driven through the conversational canvas agent (the F39 assistant with board tools), plus the diagram-as-code round-trip: Mermaid IMPORT (paste flowchart source into the assistant; parsed directly, no AI round-trip - `@hc/whiteboard` `mermaidToDiagram`) and EXPORT (the board toolbar's "Copy as Mermaid", selection- or board-scoped, `diagramToMermaid`). The richer agent modes (viewport-screenshot context, streaming multi-step board actions, computation-graph cards) remain the optional deep end. The former Phases 4 and 5 (smart diagramming/interop and GPU-scale/ecosystem/enterprise) are deliberately out of scope (section 14). The new mind-map/boardview/diagram-code node types stay schema-only groundwork; large-board scale beyond the Canvas2D spatial index/LOD is not pursued.
...
| Diagram-as-code (Mermaid / PlantUML / DOT) import | Built (Mermaid) | `@hc/whiteboard` `diagram.ts` (`mermaidToDiagram`/`diagramToMermaid`/`normalizeDiagramSpec`, `diagram.test.ts`); assistant `generateDiagram`; board toolbar "Copy as Mermaid" | Paste Mermaid flowchart source into the assistant and it lands as native stickies + connectors (parsed directly, no AI call); the toolbar serializes a selection or the whole board back to Mermaid (flowchart TD/LR + labeled edges; mindmap as an indented tree). PlantUML/DOT remain. |
...
| Sticky clustering / affinity grouping / auto-grouping | Built (AI) | assistant tool `clusterStickies`; `editor.ts` `applyStickyClusters`; `boardAi.test.ts` | "Cluster the stickies by theme" groups every sticky into 2-8 AI-labeled themes, each a titled `FrameNode` behind a grid of its members, one undo step; unknown ids skipped, guidance arg supported. Manual drag-grouping unchanged. |
...
| Feature | Status | Where (code) | Notes |
| --- | --- | --- | --- |
| Starter board templates | Built | `templates.ts` `whiteboardTemplates` + `buildTemplate`; Templates menu | Eight builders emit positioned real nodes as one undo step. Limit: fixed geometry/colors/copy, English-only, no parameterization, no thumbnails/search. |
...
All board node types and properties are added to the open file format per the schema-is-contract rule: extend the `NodeType` union and `knownNodeTypes` in `packages/schema/src/schema.ts`, define the interface + Zod schema with `...nodeBaseFields, type: z.literal("...")`, add it to the `KnownNode` union and discriminated `NodeSchema`, give it a default in `factory.ts`, register a forward migration step in `migrate.ts` keyed on the source version, and bump `currentSchemaVersion`. Additive node types and optional fields need only a version bump, because older files omit them and `UnknownNode.raw` preserves a newer client's nodes losslessly (so additive rollout is safe across mixed-version clients). Two coupling rules apply to every version bump: (1) raise the Go mirror `currentSchemaVersion` in `backend/internal/persistence/file.go` in the same change, or the write boundary `persistence/validate.go` rejects the newer file with a 422 (`ErrInvalidFile`) and nothing persists (the `UnknownNode` round-trip does not save this, the rejection is on the top-level `schemaVersion` field); purely-additive bumps need no new Go migration step. (2) Append a one-line entry to the schema-version-history doc-comment above `currentSchemaVersion` in `schema.ts` (currently stale, documenting only through v7).
