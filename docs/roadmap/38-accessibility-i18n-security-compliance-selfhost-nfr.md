...
What became of the creative-depth set. It was specified as six documents (a procedural node graph, vector authoring depth, raster and painting, motion graphics, GPU rendering, and creative interop and colour) and **withdrawn in August 2026**, before any of it shipped. Two findings drove that. The demand evidence above never improved: a surfaced node graph in particular had none from a template-first audience, which its own spec conceded. And when the first phase was actually attempted, its stated prerequisites turned out to be defect repairs rather than groundwork: masks did not render in any of the four output paths, group opacity multiplied down per child so every overlap in a semi-transparent group showed a seam, and booleans drew curved results as polylines. Those were fixed and kept. The procedural machinery built on top of them (an evaluator package, an embedded JavaScript evaluator inside the Go binary, a cross-runtime conformance corpus, and a `NodeGraph` payload in the open format) was removed rather than shipped, because a schema addition is permanent under the zero-data-loss rule and the moment before it ships is the only free moment to decline it.

None of this says creative depth is worthless. Vector authoring depth and colour management in particular stand on their own and would deserve fresh specs on their own evidence. It says the set was bundled too early, and that pulling the defect repairs out of it was the part with a demonstrable payoff.
...
- FR-9 (shipped, minus the translations themselves): Every user-visible string the tooling can safely reach is externalized. 2214 call sites (848 JSX text nodes, 692 spoken attributes, 674 expressions such as ternaries, toast calls and object fields) across the components, pages, lib and store trees resolve through `tr("key")` against a 2527-key catalog at `frontend/src/locales/en.json`. `scripts/i18n-extract.mjs` did the mechanical work and stays in the repo, because it is also the definition of what counts as a translatable string and the CI check runs it to enforce that.
  ...
  Server-composed problem+json now localizes as well. It could not before for a structural reason: `Problem()` took no code, so 268 of the 272 error responses shipped English prose with nothing a client could key off. Every response now carries a stable code, the uncoded helper is DELETED so an unlocalizable error is unrepresentable rather than merely discouraged, and the client resolves `errors.api_<code>`, falling back to the English detail for a code it does not know. The catalog value is a user-facing message rather than a translation of the developer detail, so an internal failure reads "Something went wrong" instead of a literal "invalid body"; the code, not the prose, carries the precision for logs and support. `backend/internal/httpapi/problem_code_test.go` enforces the Go side (code present, literal, and never reused for two different messages) and `i18n.catalog.test.ts` enforces the seam in both directions, since a new coded error otherwise compiles, passes every Go test, and still reaches users in English.
...
- FR-11 (shipped): Translator contribution is a JSON file dropped into `public/locales/`, documented for translators rather than for developers, with partial catalogs explicitly supported. Pseudo-localization ships as `?pseudo=1`: every untranslated string renders accented between `«` and `»` and padded ~35% longer, which exposes both failures a translated build hits and an English build never does, a hard-coded string that stays plain ASCII, and a layout that only fits English. Interpolation placeholders are left intact so the pseudo-locale exercises substitution rather than breaking it. Missing keys fall back to the base locale per key.
  ...
  CI enforcement is `i18n.catalog.test.ts`: it runs the extractor in dry mode and fails on any hard-coded user-visible string it CAN SEE, and separately checks that every `tr()` key exists in the catalog, that no orphaned keys remain, and that no value is blank. The ratchet was mutation-tested by reintroducing a hard-coded `title` and confirming the failure.

  **A blind spot found and closed, August 2026.** The ratchet is only as wide as the extractor, and the extractor's JSX text-run pattern excluded `;`, `=`, `"`, `'` and a backtick from the run itself. The stated reason, that those characters "never appear in a plain JSX text node", is false for prose: English UI text uses semicolons and apostrophes, and every HTML entity ends in `;`. Such a node never matched, so it was not even counted as skipped, and the ratchet reported a clean run over strings it had never examined. **27** user-visible strings had survived there, across the editor panels, history, print and publish dialogs, whiteboard, video surface, settings, installation, and the invite/shared/verify pages.

  All 27 are now externalized, with translations in all seven full catalogs (3232 keys each). The fix in `scripts/i18n-extract.mjs` is `unseenProse()`, a DETECTION-only sweep relaxed on exactly those five characters: it reports and never rewrites, because widening the rewrite risks a bad automated edit across hundreds of files. Its count is a fourth term in the dry-run summary and is asserted in `i18n.catalog.test.ts`. Two things the fix had to get right: the sweep must run BEFORE the `nText + nAttr + nExpr === 0` early exit, or it never runs on a clean file (it did not, until a mutation test caught it), and a mutation test reintroducing a semicolon-bearing string must fail the ratchet, which it now does.
