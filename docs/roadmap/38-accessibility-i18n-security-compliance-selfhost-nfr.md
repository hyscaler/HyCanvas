...
- FR-29 (shipped): First-run setup happens in the browser. An unconfigured server (no `DATABASE_URL`) boots into a setup mode serving an installation wizard at `/installation/step-1`, gated by a one-time access secret shown on the operator's terminal; the wizard validates Postgres, storage, and SMTP answers live with visible progress, writes the configuration, migrates the database, hands over to the normal server in-process, and creates the first account. An interactive first start offers a CLI wizard alternative (same questions and validation, in the terminal). The binary also self-daemonizes (`hycanvas service start|stop|restart|status|log`) with no OS service manager required, and `hycanvas storage migrate` moves local-disk blobs to S3-compatible storage (idempotent copy, verification, `.env` flip) without database changes.
- FR-30 (shipped): Public instances can cap one user's total upload storage across all their workspaces via `USER_STORAGE_QUOTA_BYTES` (unset = unlimited), independent of the per-workspace `ASSET_QUOTA_BYTES`; exceeding it returns a distinct 413 and the uploads panel shows the account-level usage meter.
