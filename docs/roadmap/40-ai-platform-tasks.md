...
Deliberately not done here: widow control through a non-breaking space (neither renderer treats U+00A0 as unbreakable, so it would be an ineffective hack; it needs a run-level no-break attribute); real list markers in composed text (the text engine draws them, the Go export does not, so glyph bullets stay for renderer parity until the exporter catches up); images in API-composed decks (E20's report names the regions; the server-side image job is Tier 2 of the enterprise assessment).
