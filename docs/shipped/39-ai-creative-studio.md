...
- Phase 1 (SHIPPED): Structured-output foundation and the layout engine. Landed as the pure `@hc/aistudio` package: `normalizeDesignSpec` + `designSpecJsonSchema` validate a role/layout spec (FR-12, with the legacy fraction parser kept as the degraded fallback since `oc.aiText` is free-text only); `layoutDesign(spec, size)` is the deterministic layout engine owning margins, the type scale, vertical rhythm, alignment, z-order, and WCAG-readable text color (FR-13); `qualityCheck(page)` verifies contrast/overflow/overlap (FR-14). Single-page Magic Design generated roles + a layout intent and laid them out through the engine, with a quality pass before display; its dedicated `MagicDesignPanel` was later removed in the panel-to-chat consolidation (see the note after Phase 5), and the store's `buildAiDesign` action remains without a UI caller. Multi-page, the assistant, and Magic transforms remain in later phases.
- Phase 2 (SHIPPED): Multi-page generation, outline-first. `@hc/aistudio` outline core (`normalizeOutline`/`outlineJsonSchema`/`outlineSystemPrompt`, `outlineItemToSpec` per visual role, `deckThemes` variations, `layoutDeck`); store `buildDeckFromOutline` (whole-document, one undo). The dedicated studio UI (prompt -> editable outline with reorder/edit/add/remove -> generate, with an N-way style switcher) was removed in the panel-to-chat consolidation; generation now runs through the assistant's `generateDesign` tool without an outline review step (its restoration is tracked as T09 in `../roadmap/28-presentations-leverage-tasks.md`). Brand palette grounds both the outline prompt and the themes.
...
- Phase 4 (SHIPPED): Magic transforms. `@hc/aistudio` transform core (`deriveOutline` from any current design, `switchOutline` deck->doc/social-set/poster, `recomposeSpec` = size-aware re-layout for AI Resize, `normalizeChartSpec`/`chartSystemPrompt` for Magic Charts, `paletteTheme` for style transfer). The dedicated Magic Switch and chart panels were removed in the panel-to-chat consolidation: charts remain reachable through the assistant's `insertChart` tool (-> `insertChartData`), while the transform cores (`deriveOutline`, `switchOutline`, `recomposeSpec`, `paletteTheme`) currently have no UI caller.
- Phase 5 (SHIPPED): Generative media in context (`groundImagePrompt` grounds image gen in brand palette + aspect, applied in the image generator, FR-23); critique-and-improve (FR-15; the one-click improve panel was removed in the panel-to-chat consolidation - critique remains a read-only assistant action and `harmonize`/`tidyLayout` remain assistant tools, but the combined one-turn improve flow has no dedicated surface); Magic Animate (existing `magicAnimatePage`, also an assistant tool, FR-22); RTL/non-Latin hardening (layout `dir` honors RTL alignment + paragraph direction). Streaming and a server-side job registry for generation remain future work - see note below.

Panel-to-chat consolidation (post-ship): the five dedicated AI panels (Magic Design, the outline-first studio, Magic Switch, the chart panel, and improve) were deleted in favor of ONE assistant chat driving the same capabilities through the validated tool catalog. The pure cores those panels exercised still ship in `@hc/aistudio` and several are currently orphaned - `deriveOutline`, `switchOutline`, `recomposeSpec`, and `paletteTheme` have no caller, the store's `buildAiDesign` has no UI caller, and `qualityCheck` is computed per generated page but its report is not surfaced. They are kept deliberately: the F28 leverage plan (`../roadmap/28-presentations-leverage-tasks.md`) reconnects the outline flow (T09) and the quality gate (T12) rather than rebuilding them.
