  ...
  return (
    <div className="flex h-full flex-col" data-testid="reading-order-pane">
      <div className="flex items-center justify-between border-b border-neutral-200 px-3 py-2">
        <span className="text-xs font-semibold uppercase tracking-wide text-neutral-500">Reading order</span>
        {explicit && (
          <button
            type="button"
            onClick={() => useEditor.getState().resetReadingOrder()}
            title="Reset to layer order"
      ...
      <div className="flex-1 overflow-auto py-1">
        {ordered.length === 0 && (
          <div className="px-3 py-3 text-sm text-neutral-400">
            <div className="font-medium text-neutral-500">Nothing to read</div>
        ...
        {ordered.map((node, i) => {
          ...
          return (
            ...
            >
              ...
              <span className="w-5 shrink-0 text-right text-[11px] tabular-nums text-neutral-400">{i + 1}</span>
              <span className="min-w-0 flex-1 truncate">
                <span className="truncate">{node.name || node.type}</span>
                {alt && !decorative && (
                  <span className="ml-1.5 truncate text-[11px] text-neutral-400" title={alt}>
                    {alt}
                  </span>
                )}
              </span>
