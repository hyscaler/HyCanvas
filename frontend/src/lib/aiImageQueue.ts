...
// One settle report per drain, per design: counts since the last report.
  ...
  while (running < maxConcurrent && queue.length) {
    ...
    void resolveTask(task)
      ...
      .finally(() => {
        ...
        else if (running === 0) flushTally();
      });
  }
}

function flushTally(): void {
  for (const [designId, t] of tally) {
...
async function resolveTask(task: AiImageTask): Promise<boolean> {
  ...
  // Attribution-required hits are SKIPPED: an automatic background insertion
  // cannot guarantee the credit compiles, so only license-free assets qualify
  // here; the stock panel remains the surface for attributed inserts.
  if (!url && routeImageSource(task.prompt) === "stock") {
    try {
      const hits = await oc.stockSearch(task.prompt, "photo", { limit: 5 });
      const free = hits.find((h) => !h.license?.attributionRequired && (h.sourceUrl || h.previewUrl));
      if (free) url = free.sourceUrl || free.previewUrl;
    } catch {
