// The built-in theme catalog (F40 E10): pure data, one source of truth for
// every consumer - the editor's theme picker (localized display names wrap
// these ids in the frontend), headless generation (composeDeckFile resolves a
// themeId here), and the Go server's embedded manifest (scripts/
// gen-theme-catalog.mjs, validated by a parity test). Colors are the six T19
// slots IN ORDER [primary, accent, deep, tint, ink, paper]; every entry must
// pass themeContrastFailures (a catalog test walks all of them), so a theme a
// user picks can never produce unreadable text.

export type ThemeStyleGroup = "professional" | "editorial" | "bold" | "minimal" | "warm" | "tech" | "dark";

export interface ThemeCatalogEntry {
  id: string;
  /** English display name; the frontend shows a localized name keyed on id. */
  name: string;
  style: ThemeStyleGroup;
  /** Six hexes in slot order: primary, accent, deep, tint, ink, paper. */
...
/** Catalog lookup by id, or null. */
export function themeCatalogEntry(id: string): ThemeCatalogEntry | null {
  return themeCatalog.find((t) => t.id === id) ?? null;
}
