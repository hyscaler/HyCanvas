...
// words fade in, removed words fade out. Positions come from the SAME pure
// text layout both renderers use (approximateMeasure), so from- and to-side
// measurements are consistent by construction - which is all a morph needs.
//
...
import { approximateMeasure, layoutText } from "@hc/text";
...
export function wordBoxes(node: TextNode): WordBox[] {
  const laid = layoutText(node);
  const pad = (node.box.padding ?? { t: 0, r: 0, b: 0, l: 0 }) as { l: number; r: number };
  const contentW = Math.max(0, node.box.width - pad.l - pad.r);
  const out: WordBox[] = [];
  for (const line of laid.lines) {
    const colLeft = line.colLeft ?? 0;
    const lineW = line.width;
    const alignOff =
      line.align === "center" ? Math.max(0, ((line.colWidth ?? contentW) - lineW) / 2)
      : line.align === "right" ? Math.max(0, (line.colWidth ?? contentW) - lineW)
      : 0;
    let x = pad.l + colLeft + line.x + alignOff;
    for (const seg of line.segments) {
      ...
      for (const part of parts) {
        if (!part) continue;
        const w = approximateMeasure(part, seg.style);
        if (part.trim()) {
          out.push({ text: part, x, y: line.y, width: w, lineHeight: line.height, style: seg.style });
        }
        x += w;
      }
    }
 ...
 * of fades).
 */
export function planWordMorph(from: TextNode, to: TextNode, cap = maxMorphWords): WordMorphPlan | null {
  const a = wordBoxes(from);
  const b = wordBoxes(to);
